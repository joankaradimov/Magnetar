...
#define FUNCTION_PATCH(destination, replacement) \
        FunctionPatch CONCAT(function_patch_, __LINE__, __COUNTER__)(destination, replacement)

#define FAIL_STUB_PATCH(destination) \
        FailStubPatch CONCAT(fail_stub_patch_, __LINE__, __COUNTER__)(destination)

#define CALL_SITE_PATCH(destination, function) \
        CallSitePatch CONCAT(call_site_patch_, __LINE__, __COUNTER__)(destination, function)

#define NOP_PATCH(destination, block_length) \
        NopPatch CONCAT(nop_patch_, __LINE__, __COUNTER__)(destination, block_length)

#define MEMORY_PATCH(destination, data) \
        MemoryPatch CONCAT(memory_patch_, __LINE__, __COUNTER__)(destination, data)
